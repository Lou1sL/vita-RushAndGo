	...
	for (i = 0; i < 8; ++i) {
		...
		for (j = 0; j < 8; ++j) {
			...
			if ((*font & (128 >> j))) {
				...
				draw_pixel(pos_x + 2, pos_y + 0, color);
				draw_pixel(pos_x + 3, pos_y + 0, color);

                draw_pixel(pos_x + 0, pos_y + 1, color);
                draw_pixel(pos_x + 1, pos_y + 1, color);
                draw_pixel(pos_x + 2, pos_y + 1, color);
                draw_pixel(pos_x + 3, pos_y + 1, color);

                draw_pixel(pos_x + 0, pos_y + 2, color);
                draw_pixel(pos_x + 1, pos_y + 2, color);
                draw_pixel(pos_x + 2, pos_y + 2, color);
                draw_pixel(pos_x + 3, pos_y + 2, color);

                draw_pixel(pos_x + 0, pos_y + 3, color);
                draw_pixel(pos_x + 1, pos_y + 3, color);
                draw_pixel(pos_x + 2, pos_y + 3, color);
                draw_pixel(pos_x + 3, pos_y + 3, color);
...
void font_draw_char_small(int x, int y, uint32_t color, char c)
{
	unsigned char *font = (unsigned char *)(msx_font + (c - (uint32_t)' ') * 8);
	int i, j, pos_x, pos_y;
	for (i = 0; i < 8; ++i) {
...
void font_draw_string_small(int x, int y, uint32_t color, const char *string)
{
	if (string == NULL) return;
	...
	while (*s) {
		if (*s == '\n') {
			...
			font_draw_char_small(x, y, color, *s);
			x += 16;
		}
		++s;
	}
...
void font_draw_stringf(int x, int y, uint32_t color, const char *s, ...)
{
	...
	va_end(argptr);
	font_draw_string_small(x, y, color, buf);
}

void font_draw_stringf_big(int x, int y, uint32_t color, const char *s, ...)
{
	char buf[256];
	va_list argptr;
	va_start(argptr, s);
